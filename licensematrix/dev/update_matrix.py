...
SPDX = load(open(THISDIR + "/spdx.json"))["licenses"]

license_mat = {}
for line in open(THISDIR + "/licenselist.txt").readlines():
	...
	cannot = [must["attribute"]["title"] for must in modules["summary"]["cannot"]]
...
# Write to file
dump(license_mat, open(THISDIR + "/license_matrix.json", "w"))
